			    ...
			    void *arg)
{
	...
	bcast_item_b = glist_entry(b,
				   struct dbus_bcast_item,
				   dbus_bcast_q);
	return timespec_diff(&bcast_item_a->next_bcast_time,
					...
					int count)
{
	...
	timespec_add_nsecs(bcast_interval*100,
				&new_bcast->next_bcast_time);
	...
	while (1) {
		...
		glist_for_each_safe(glist, glistn, &dbus_broadcast_list) {
		   struct dbus_bcast_item *bcast_item = glist_entry(glist,
							...
							dbus_bcast_q);
		   now(&current_time);
		   tdiff = timespec_diff(&bcast_item->next_bcast_time,
					 &current_time);
		   ...
		   if (tdiff/100 >= (bcast_item->bcast_interval) &&
			...
			timespec_add_nsecs(bcast_item->bcast_interval*100,
