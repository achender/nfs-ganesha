                              ...
                              struct fsal_obj_handle **handle)
{
	...
	*handle = NULL; /* poison it */
	...
	myself = container_of(dir_hdl, struct pt_fsal_obj_handle, obj_handle);
        ...
        attrib->mask = dir_hdl->export->ops->fs_supported_attrs(dir_hdl->export);
        status = PTFSAL_mknode(dir_hdl, name, opctx, attrib->mode, nodetype, dev, fh, attrib);
	if(FSAL_IS_ERROR(status))
		return status;
                                 ...
                                 struct fsal_obj_handle **handle)
{
	...
	*handle = NULL; /* poison it first */
	...
	myself = container_of(dir_hdl, struct pt_fsal_obj_handle, obj_handle);
        ...
        attrib->mask = dir_hdl->export->ops->fs_supported_attrs(dir_hdl->export);
        status = PTFSAL_symlink(dir_hdl, name, link_path, opctx,
...

{
	fsal_errors_t fsal_error = ERR_FSAL_NO_ERROR;
	int retval = 0;
	struct pt_fsal_obj_handle *myself = NULL;
	int mntfd;
        fsal_status_t status;
	...
	if(refresh) { /* lazy load or LRU'd storage */
		...
		mntfd = pt_get_root_fd(obj_hdl->export);
                ...
                status =  PTFSAL_readlink(obj_hdl, opctx, link_buff,
                                            &retlink, NULL);
		if(FSAL_IS_ERROR(status))
			return(status);
			      ...
			      const char *name)
{
	...
	myself = container_of(obj_hdl, struct pt_fsal_obj_handle, obj_handle);
        ...
        status = PTFSAL_link(destdir_hdl, myself->handle, name, opctx, NULL);
...

{
	...
	if(dirfd < 0) {
        ...
        nb_entries = 0;
        max_dir_entries = sizeof(buf) / sizeof(fsal_dir_t);
        ...
        while(nb_entries < max_dir_entries) {
          /***********************/
          /* read the next entry */
          /***********************/

          readdir_rc = ptfsal_readdir(opctx, dir_hdl->export, &dir_desc, &buffstat, fsi_dname);
                      ...
                      struct fsal_obj_handle *other_hdl)
{
	...
	if((obj_hdl->type != other_hdl->type) ||
	   (myself->handle->data.handle.handle_type != other->handle->data.handle.handle_type) ||
	   (myself->handle->data.handle.handle_size != other->handle->data.handle.handle_size))
		return false;
			      ...
			      struct fsal_obj_handle **handle)
{
	...
	basepart++;
	fsal_status = fsal_internal_get_handle_at(opctx, exp_hdl, dir_fd, basepart, fh);
	if(FSAL_IS_ERROR(fsal_status)) {
		goto fileerr;
        }
	...
	//retval = fstatat(dir_fd, basepart, &stat, AT_SYMLINK_NOFOLLOW);
        retval = ptfsal_stat_by_name(opctx,
                    exp_hdl,
                    basepart,
                    &p_stat);
				...
				struct fsal_obj_handle **handle)
{
	...
	*handle = NULL; /* poison it first */
	...
	if(attrib.type == SYMBOLIC_LINK) { /* I could lazy eval this... */
                ...
                status = fsal_readlink_by_handle(opctx, exp_hdl, fh,
                                                 link_buff, retlink);
		if(FSAL_IS_ERROR(status))
			return(status);
