...

{
	...
	do {
		...
		for(bpos = 0; bpos < nread;) {
			...
			memcpy(&entry_cookie->data.cookie, &dentry->d_off, sizeof(off_t));

			/* callback to cache inode */
			if(!rcb(opctx, dentry->d_name,
                                    dir_state,
...
static fsal_status_t release(struct fsal_obj_handle *obj_hdl)
{
	...
	if(obj_hdl->type == SYMBOLIC_LINK) {
		if(myself->u.symlink.link_content != NULL)
			free(myself->u.symlink.link_content);
	} else if(pt_unopenable_type(obj_hdl->type)) {
				...
				struct fsal_obj_handle **handle)
{
	...
	ptfsal_handle_t  *fh;
        struct attrlist attrib;
	int mount_fd = pt_get_root_fd(exp_hdl);
	char *link_content = NULL;
