...
ptfsal_threadcontext_t *ptfsal_get_thread_context()
{
  ...
  if (p_cur_context == NULL) {
    ...
    FSI_TRACE(FSI_NOTICE, "malloc %lu bytes fsal specific data", sizeof(ptfsal_threadcontext_t));
    if (p_cur_context != NULL) {
                         ...
                         int               * handle_index) /*OUT*/
{
  ...
  if (g_ptfsal_context_flag) {
    ...
    if (p_cur_context != NULL && p_cur_context->cur_namecache_handle_index != -1) {
                          ...
                          struct stat                  * p_stat)
{
  ...
  if (g_ptfsal_context_flag) {
    ptfsal_threadcontext_t *p_cur_context;
    p_cur_context = ptfsal_get_thread_context();
    if (p_cur_context != NULL && p_cur_context->cur_fsi_handle_index != -1 ) {
...

{
  ...
  if (handleOpened >=0) {
    ...
    rc = ptfsal_name_to_handle(p_context, p_parent_directory_handle->obj_handle.export, fsal_path, p_object_handle);
    if (rc != 0) {
            ...
            int             in_handle)
{
  ...
  while (cur_size > 0) {
    ...
    rc = CCL_PREAD(&ccl_context, &buf[buf_offset], read_amount, cur_offset, 
             ...
             int           in_handle)
{
  ...
  while (cur_size > 0) {
    ...
    bytes_written = CCL_PWRITE(&ccl_context, in_handle, &buf[buf_offset], write_amount, 
                    cur_offset);
    if (bytes_written < 0) {
                      ...
                      ptfsal_handle_t     * p_handle)
{
  ...
  rc = ptfsal_stat_by_name(p_context,
                           export,
                           p_fsalpath,
                           &fsi_stat);
  ...
  memset(p_handle->data.handle.f_handle, 0, sizeof(p_handle->data.handle.f_handle));
  ...
  p_fsi_handle->data.handle.handle_type     = posix2fsal_type(fsi_stat.st_mode);
