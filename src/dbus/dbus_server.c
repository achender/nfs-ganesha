 ...
 * @param arg: Pointer to a timespec.  The dbus_bcast_item
 *             whose last_bcast_time is closer should appear
			    ...
			    void *arg)
{
	...
	nsecs_elapsed_t tdiff_a;
	nsecs_elapsed_t tdiff_b;
	struct timespec *now = (struct timespec *)arg;
	...
	bcast_item_b = glist_entry(b,
				   struct dbus_bcast_item,
				   dbus_bcast_q);
	tdiff_a = timespec_diff(now,
				&bcast_item_a->last_bcast_time);
	tdiff_b = timespec_diff(now,
				&bcast_item_a->last_bcast_time);

	if (tdiff_a == tdiff_b)
		return 0;
	else if (tdiff_a < tdiff_b)
		return -1;
	/*else tdiff_a is greater that tdiff_b*/

	return 1;
}

/*
 * @brief del_dbus_broadcast: Delete a callback from the broadcast service
 *
 * Function to be called by any thread wanting to remove a callback
 ...
 * @param bcast_interval: The time in usec between calls
					...
					int count)
{
	...
	memset(&(new_bcast->last_bcast_time), 0, sizeof(struct timespec));
	...
	while (1) {
		...
		glist_for_each_safe(glist, glistn, &dbus_broadcast_list) {
		   struct dbus_bcast_item *bcast_item = glist_entry(glist,
							...
							dbus_bcast_q);
		   now(&current_time);
		   tdiff = timespec_diff(&current_time,
					 &bcast_item->last_bcast_time);

		   if (tdiff/100 >= (bcast_item->bcast_interval) &&
		       (bcast_item->count > 0 ||
		       bcast_item->count == BCAST_FOREVER)) {

			bcast_item->last_bcast_time = current_time;
			rc = bcast_item->bcast_callback(
							bcast_item->bcast_arg);
		   ...
		   } else /* list is sorted by soonest to longest */
			break;
		}
		pthread_mutex_unlock(&dbus_bcast_lock);
 ...
 *
 * Function to be called by a threads callback routine
