                                        ...
                                        int *pfd, int oflags)
{
  ...
  if (myself->handle->data.handle.handle_type != DIRECTORY) {
    ...
    open_rc = ptfsal_open_by_handle(p_context, myself, oflags, 0777);
  } else {
    ...
    open_rc = ptfsal_opendir(p_context, myself->obj_handle.export, fsi_name, NULL, 0); 
  }
 ...
 * \return status of operation
 */
fsal_status_t fsal_internal_get_handle_at(const struct req_op_context *p_context,
                                          struct fsal_export *export,
                                          int dfd, const char *p_fsalname, /* IN */
                                          ptfsal_handle_t *p_handle)/* OUT */
{
  fsi_stat_struct buffstat;
  int stat_rc;
  ...
  if(stat_rc == 0) {
    ...
    p_handle->data.handle.handle_key_size = OPENHANDLE_KEY_LEN;
    p_handle->data.handle.handle_version = OPENHANDLE_VERSION;
    FSI_TRACE(FSI_DEBUG, "Handle=%s", p_handle->data.handle.f_handle);    
  } else {
    return fsalstat(ERR_FSAL_NOENT, errno);
  }
