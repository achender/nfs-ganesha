				    ...
				    struct gsh_buffdesc *fh_desc)
{
	...
	fh_desc->len = hdl->data.handle.handle_key_size;  /* pass back the key size */
...
static void
pt_verifier(struct gsh_buffdesc *verf_desc)
{
        memcpy(verf_desc->addr, &PT_write_verifier, verf_desc->len);
}

void
set_pt_verifier(verifier4 *verifier)
{
				...
				struct fsal_export **export)
{
	...
	*export = NULL; /* poison it first */
	...
	myself->root_fd = -1;
        ...
        LogInfo(COMPONENT_FSAL, "PT get version is %d options 0x%X",
	...
	if(myself->root_fd < 0) {
		...
		status = fsal_internal_get_handle_at(NULL, &myself->export,  myself->root_fd, NULL, fh);
